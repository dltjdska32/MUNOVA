...
DDD(Domain-Driven Design) 및 클린 아키텍처 기반으로 설계하여 확장 가능하고 유지보수가 용이한 시스템을 구현했습니다.

### 👨‍💻 담당 역할
**상품 도메인 전담 개발**
- Hexagonal Architecture(Port & Adapter) 기반 설계 및 구현
- 분산 시스템 환경에서 데이터 일관성 보장 (Transactional Outbox 패턴)
- Redis 기반 고성능 동시성 제어 및 캐싱 전략 수립
- Spring Batch를 활용한 대용량 통계 데이터 동기화
